        ...
        <div class="row g-2 align-items-end">
            ...
            <div class="col-md">
                <select name="job_categories[]" id="job_categories" class="form-select" required>
                    <option value="all" {{ in_array('all', session('searchInput.job_categories', [])) ? 'selected' : '' }}>
                        Tous les praticiens
                    </option>
                    @foreach ($jobcategories as $category)
                        <option value="{{ $category->id }}"
                            {{ in_array($category->id, session('searchInput.job_categories', [])) ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md">
                <select name="animal_categories[]" id="animal_categories" class="form-select" required>
                    <option value="all" {{ in_array('all', session('searchInput.animal_categories', [])) ? 'selected' : '' }}>
                        Tous les animaux
                    </option>
                    @foreach ($animalcategories as $category)
                        <option value="{{ $category->id }}"
                            {{ in_array($category->id, session('searchInput.animal_categories', [])) ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md">
                <select name="departments[]" id="departments" class="form-select" required>
                    <option value="all" {{ in_array('all', session('searchInput.departments', [])) ? 'selected' : '' }}>
                        Tous les départements
                    </option>
                    @foreach ($department as $dep)
                        <option value="{{ $dep->id }}"
                            {{ in_array($dep->id, session('searchInput.departments', [])) ? 'selected' : '' }}>
                            {{ $dep->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-auto">
                <button type="submit" class="btn btn-primary btn-custom" aria-label="Rechercher des professionnels selon les critères sélectionnés">
