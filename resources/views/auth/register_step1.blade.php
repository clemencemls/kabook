                       ...
                       <section class="d-flex justify-content-center align-items-center vh-100">
                          <div class="container border py-5 px-5 bloclogin rounded-5" style="max-width: 500px;">
                            ...
                            @csrf
                              ...
                              <div class="mb-3 text-start">
                                <label for="phone" class="form-label">A quel numéro de téléphone peut-on vous joindre ?</label>
                                <input type="phone" class="form-control" id="phone" name="phone" required />

                              </div>
                              <label for="job_categories">Quelles sont vos activités ?</label>
        <select name="job_categories[]" id="job_categories" multiple class="form-select">
            @foreach ($jobcategories as $category)
                <option value="{{ $category->id }}">{{ $category->name }}</option>
            @endforeach
        </select>

        <label for="animal_categories">Quels animaux prenez-vous en charge ?</label>
    <select name="animal_categories[]" id="animal_categories" multiple class="form-select">
